        ...
        user.insert(
            {
                ...
                "queues": req_data["queues"],
                "token": req_data["token"],
                "refresh": req_data["refresh"],
                "ExperiesIn": req_data["ExpiresIn"],
            }
        )
        return (
            "User: "
...
def updateUser(username, attributToUpdate, newValue):
    ...
    if existing_user:
        db.user.update({"username": username}, {"$set": {attributToUpdate: newValue}})
    else:
...
def createQueue(username, queue_data):
    ...
    if existing_queue is None:
        if existing_user:
            ...
            db.user.update(
                {"username": username}, {"$set": {"queues": existing_user["queues"]}}
            )
            ...
            db.queue.insert(
                {
                    ...
                    "orders": queue_data["orders"],
                    "cutoff_amt": queue_data["cutoff_amt"],
                }
            )
        else:
            return "error: user does not exist"
...
def dequeue(queueid):
    ...
    if existing_queue:
        ...
        if not len(existing_queue["orders"]) == 0:
            ...
            db.queue.update(
                {"queueid": queueid}, {"$set": {"orders": existing_queue["orders"]}}
            )
...
def deleteQueue(username, queueid):
    ...
    if existing_user:
        ...
        # deletes the queue
        db.queue.delete_one({"queueid": queueid})
